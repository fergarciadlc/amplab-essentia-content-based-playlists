...
from audio_analysis.extractors.embeddings import EmbeddingModel, VGGishModel
...
class VGGishVoiceInstrumentalExtractor(FeatureExtractor):
    def __init__(
        ...
        self.model = es.TensorflowPredict2D(
            graphFilename=self.model_weights, output="model/Softmax"
        )
    ...
    def extract(self, audio_data: AudioData) -> Dict[str, str]:
        ...
        return {
            "vggish_voice": float(predictions_by_class["voice"]),
            "vggish_instrumental": float(predictions_by_class["instrumental"]),
        }
...
Usage:
    ...
    genre_extractor = VGGishVoiceInstrumentalExtractor(
        embedding_model=VGGishModel(
            model_weights="audio_analysis/model_weights/audioset-vggish-3.pb",
            model_metadata="audio_analysis/model_metadata/voice_instrumental-audioset-vggish-1.json",
        ),
        model_weights="audio_analysis/model_weights/voice_instrumental-audioset-vggish-1.pb",
        model_metadata="audio_analysis/model_metadata/voice_instrumental-audioset-vggish-1.json",
    )
