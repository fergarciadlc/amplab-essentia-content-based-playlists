...
class DanceabilityExtractor(FeatureExtractor):
    def __init__(
        ...
        model_weights: str = "audio_analysis/model_weights/danceability-discogs-effnet-1.pb",
        model_metadata: str = "audio_analysis/model_metadata/danceability-discogs-effnet-1.json",
    ):
    ...
    def extract(self, audio_data: AudioData, audio_embeddings=None) -> Dict[str, str]:
        ...
        return {
            "effnet_discogs_danceable": float(predictions_by_class["danceable"]),
            "effnet_discogs_not_danceable": float(
                predictions_by_class["not_danceable"]
            ),
        }
...
Usage:
    cd src
    python -m audio_analysis.extractors.danceability [--audio_file path/to/audio/file]
    """
    )
    import json
    ...
    genre_extractor = DanceabilityExtractor(
